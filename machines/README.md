...
Machines are independent system configurations for physical or virtual machines. Each machine is a
real `nixosConfigurations.<hostname>` entry in the single root `flake.nix`, generated from its
`machines/<hostname>/` directory: `configuration.nix`, `hardware-configuration.nix` and local sops
encrypted secrets `args.enc.json`. The `nixos-config` repo is a single flake with supporting options,
modules and profiles that are used to compose the different machines being managed - there is nothing
machine-specific to stage or link at the repo root; `nix build .#<hostname>` (or `clu build
<hostname>`) selects the machine directly as a flake attribute. At the top level of the repo there are
reusable shared flake arguments that can be overridden at the machine level if specific customization
is desired. This setup makes it both reusable, composable and customizable per machine as needed while
...
* `flake.nix` / `flake.lock` - the single shared flake for every machine (only macbook needs an extra
...
* `configuration.nix` for the main machine configuration
...
`machines/<hostname>/args.enc.json`) to `args.dec.json` and git-stages them (`git add -f`) - Nix
...
module option at evaluation time) don't need this treatment at all - see how `machine.smb.secrets` /
`options/services/raw/smb` already renders SMB credentials via `sops.templates`, decrypted straight to
`/etc/smb/secrets/<share>` at activation by sops-nix, the same as `machine.secrets` does for the user
password hash.
...
| ------------------------- | ----- | ----------------- | --------------------------------------
| `target`                  | str   | `xfce/desktop`    | Only used by `nixosConfigurations.install` to bootstrap a brand new machine before it has a `machines/<hostname>` directory
| `hostname`                | str   | `nixos`           | Not settable for real machines - always the `machines/<hostname>` directory name; this default only applies to `nixosConfigurations.install`/`iso`
