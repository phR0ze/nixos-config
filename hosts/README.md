...
encrypted secrets `args.enc.yaml`. The `nixos-config` repo is a single flake with supporting options,
...
* `args.enc.yaml` - sops encrypted arguments to be shared by all hosts or overridden locally
...
* `args.enc.yaml` sops encrypted arguments which override `nixos-config/args.enc.yaml`
* `secrets.enc.yaml` runtime secrets decrypted by sops-nix at activation (never touches the Nix store)

## Build-time args
Before evaluation, `clu` decrypts just the target host's build-time args (root `args.enc.yaml` +
`hosts/<hostname>/args.enc.yaml`) to `args.dec.yaml` and git-stages them (`git add -f`) - Nix
...
`args.enc.yaml` argument files provide a mechanism for rapidly specifying bulk reusable configuration 
across many hosts.

### Accepted args.enc.yaml values
