...
let
  ...
  machine-learning-tag = if (gpu.nvidia) then "${cfg.tag}-cuda"
...
in
{
  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      assertions = [
        #{ assertion = (cfg ? "debug"); message = "echo '${builtins.toJSON args}' | jq"; }
      ...
      virtualisation.oci-containers.containers."${cfg.name}-machine-learning" = {
        ...
        image = "ghcr.io/immich-app/immich-machine-learning:${machine-learning-tag}";
        autoStart = true;
        #user = cfg.user.name;
