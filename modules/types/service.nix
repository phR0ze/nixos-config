# Declares the option set shared by every `services.oci.<name>` module
#
# ### Usage
# Spliced into a module's own options with `//`, exactly the way a native service declares its
# options inline (see modules/services/native/jellyfin.nix):
#
# ```nix
...
# ```
#
# `defaults` is the module's own static defaults - never host args. Anything a host needs to
# override comes either from its `configuration.nix` or from `host.services.oci.<name>.*`,
# forwarded by modules/default.nix's `ociForward`, the same as every native service.
#---------------------------------------------------------------------------------------------------
{ lib, defaults, ... }: with lib.types;
{
  ...
  sopsFile = lib.mkOption {
    description = lib.mdDoc ''
      ...
      that with its own `enable`-gated assertion.
    '';
