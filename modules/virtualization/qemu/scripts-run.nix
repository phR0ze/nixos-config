# QEMU guest run script
# Not a NixOS module - a plain function returning the shell script that launches the VM. Imported
# and called from `guest.nix`, which assembles the `guest.options` QEMU argument list consumed here
...
''
  ...
  # ----------------------------------------------------------------------------------------------
  [ ! -d "${guest.hostname}" ] && echo "Must be run from the flake directory" && exit 1
  VMDIR="${guest.hostname}"
