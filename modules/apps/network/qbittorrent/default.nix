...
let
  ...
  # `QFileDialog::getOpenFileNames` static call, so qBittorrent never persists its geometry across
  # runs and there is nothing in `qBittorrent.conf` to set - it comes up at Qt's cramped default size
  # on the first open of every session.
  # - the scales are applied to `baseWidth`/`baseHeight` below, *not* to the geometry read back from
  #   the window: qBittorrent does keep the chooser's last size for the life of the process, so
  #   scaling the observed size would compound on every reopen within a session
  # - the dialog is left centred on wherever it came up, then clamped so it can't be pushed off screen
  fileDialogScript = ''
    ...
    if (get_window_class() == "qBittorrent" and get_window_type() == "WINDOW_TYPE_DIALOG"
      ...
      -- Qt's default size for this chooser, measured on a stock xfce session
      local baseWidth, baseHeight = 633, 442;
      local x, y, width, height = get_window_geometry();
      local screenWidth, screenHeight = get_screen_geometry();
      local newWidth = math.min(math.floor(baseWidth * ${toString cfg.fileDialogWidthScale}), screenWidth);
      local newHeight = math.min(math.floor(baseHeight * ${toString cfg.fileDialogHeightScale}), screenHeight);
    ...
    apps.network.qbittorrent = {
      ...
      fileDialogWidthScale = lib.mkOption {
        ...
        description = lib.mdDoc ''
          ...
          also enabled. Leave both scales at `1.0` to keep the dialog at its default size.
        '';
      };
